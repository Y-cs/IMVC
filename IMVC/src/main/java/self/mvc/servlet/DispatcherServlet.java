package self.mvc.servlet;


import org.apache.commons.lang3.StringUtils;
import self.mvc.annotations.*;
import self.mvc.pojo.Handler;
...
public class DispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            if (aClass.isAnnotationPresent(RequestMapping.class)) {
                ...
                baseUrl = annotation.value(); // 等同于/self.demo
            }

            boolean openAuthority = false;
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                String url = baseUrl + methodUrl;    // 计算出来的url /self.demo/query
                ...
                handler.setOpenAuthority(openAuthority);
    ...
    private void doInstance() {
        ...
        try {
            ...
            for (int i = 0; i < classNames.size(); i++) {
                String className = classNames.get(i);  // com.lagou.self.demo.controller.DemoController
    ...
    // 扫描类
    // scanPackage: com.lagou.self.demo  package---->  磁盘上的文件夹（File）  com/lagou/self.demo
    private void doScan(String scanPackage) {
        ...
        for (File file : files) {
            if (file.isDirectory()) { // 子package
                // 递归
                doScan(scanPackage + "." + file.getName());  // com.lagou.self.demo.controller
...
//        method.invoke() //
        ...
        if (!checkUser(handler, req)) {
            resp.getWriter().write("403 you Have No Authority");
            return;
        }
    ...
    private boolean checkUser(Handler handler, HttpServletRequest req) {
        String user = req.getHeader("user");
        return handler.checkUser(user);
    }
